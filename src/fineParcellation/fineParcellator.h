...
private:
    ...
    void OutPartSizeHist(std::string fileName, int minSize, int maxSize);
...
public:
    ...
    CFineParcellator(std::string surfName, std::string fmriName, std::string mhd4fmri);
    ~CFineParcellator();
    ...
    std::vector<float>& GetParcelTimeSeries(int parcelIdx);
    ...
    void PrepareGraph4Ncut(graph_t* graph);
...
//     void PrepareGraph4Ncut(const KML::CPartition& currentPartition,Eigen::SparseMatrix<float>& sims);
    ...
    void DoNcut4WholeBrain( int nParts = 200 );
    ...
    void NormalizedCut(int numParts);
    ...
    void SaveNcutPartitions(bool toSaveVtk,  int pid = -1 );
